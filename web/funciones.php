...
-->
        ...
        {
            return $dbconn = pg_connect("host=dev.appwebpy.com port=5432 dbname=ivasys user=postgres password=postgres "
       ...
       function func_existeDato($dato, $tabla, $columna){
         ...
         function func_existeFecha($dato, $tabla, $columna){
            selectConexion('ivasys');
            $query = "select * from declaraciones where pro_cod = $dato and extract (month from dec_fecha) =extract(month from now()) ;";
            $result = pg_query($query) or die ("Error 108: ".$query);
            if (pg_num_rows($result)>0)
            {
               return true;
            } else {
