...
header("Location:http://dev.appwebpy.com/IVASYS/login/acceso.html");
...
<head>
    ...
    <title>IVASYS- Declarar IVA</title>
    <!-- Bootstrap Core CSS -->
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                      <h1 class="page-header">Declaraciones - <small>IVASYS</small></h1>
                </div>	
            </div>
            <!-- /.row -->
            <div class="row">
                        ...
                        <div class="panel-body">
                            <div class="dataTable_wrapper">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <tbody>
                    <?php
                    $query = "select * from declaraciones dec, empresas em where dec.pro_cod=em.pro_cod ;";
                    $result = pg_query($query) or die ("Error al realizar la consulta");
                    while($row1 = pg_fetch_array($result))
                    {
                        $estado=$row1["dec_estado"];
                        if($estado=='t'){$estado='Activo';}else{$estado='Inactivo';}
                        echo "<tr><td style='display:none'>".$row1["dec_cod"]."</td>";
                        echo "<td>".$row1["dec_fecha"]."</td>";
                        echo "<td>".$row1["pro_razon"]."</td>";
                        echo "<td>".$row1["dec_facturacion"]."</td>";
                        echo "<td>".$row1["dec_compras"]."</td>";
                        echo "<td>".$row1["dec_monto_imponible"]."</td>";
                        echo "<td>".$row1["dec_calculoiva"]."</td>";
                        echo "<td>".$row1["dec_monto_a_favor"]."</td>";
                        echo "<td>".$estado."</td>";
                        echo "<td>";?>
                        <a onclick='eliminar(<?php echo $row1["dec_cod"];?>)' class="btn btn-danger btn-xs active" data-toggle="modal" data-target="#modalbor" role="button">Borrar</a>
                        <?php
                        echo "</td></tr>";
                    }
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
                                    <form  autocomplete="off" class="form-horizontal" name="agregarform" action="../class/ClsDeclaraciones.php" method="post" role="form">
					<div class="form-group">
                                            <label  class="col-sm-2 control-label" for="input01">Empresa</label>
                                            <div class="col-sm-10">
                                                <select name="txtEmpresaA" class="form-control" id="txtEmpresaA" required>
                                                    <?php
                                                    //esto es para mostrar un select que trae datos de la BDD
                                                    conexionlocal();
                                                    $query = "Select pro_cod,pro_razon from empresas where pro_activo='t'";
                                        ...
                                        <div class="form-group">
                                            <label  class="col-sm-2 control-label" for="input01">Facturaciones</label>
                                            <div class="col-sm-10">
                                            <input type="number" name="txtFacturacionA" class="form-control" id="txtFacturacionA" placeholder="ingrese nombre"/>
                                            </div>
					</div>
					<div class="form-group">
                                            <label  class="col-sm-2 control-label" for="input01">Compras</label>
                                            <div class="col-sm-10">
                                            <input type="number" name="txtCompraA" class="form-control" id="txtCompraA" placeholder="ingrese una descripcion" />
                                            </div>
					</div>	
				</div>
				<!-- Modal Footer -->
	...
	<div class="modal fade" id="modalbor" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<!-- Modal Body -->
				<div class="modal-body">
                                    <form class="form-horizontal" name="borrarform" action="../class/ClsDeclaraciones.php" onsubmit="return submitForm();" method="post" role="form">
						<div class="form-group">
